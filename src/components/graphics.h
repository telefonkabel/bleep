...
namespace bleep
{
	...
	public:
		...
		int32_t width() const { return m_renderable.Sprite() ? m_renderable.Sprite()->width : 0; };
		/// \brief Getter for the sprite height.
		/// @returns The height of the sprite as int, or 0 if no sprite is found.
		int32_t height() const { return m_renderable.Sprite() ? m_renderable.Sprite()->height : 0; };
