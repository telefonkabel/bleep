...
namespace bleep
{
	...
	public:
		...
		/// @returns float of CKinetics::m_speedFactor.
		void speedFactor(float fac) { m_speedFactor = fac; };
		...
		void velocity(v2d velocity) { m_velocity = velocity; };
		...
		void acceleration(v2d acceleration) { m_acceleration = acceleration; };
		/// \brief Getter for the current plain speed.
		/// @returns Calculated speed as float.
		float speed() const { return  velocity().mag(); };
